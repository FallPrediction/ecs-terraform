resource "aws_ecs_task_definition" "general_worker" {
  family                   = "${var.service_name}-${var.environment}-general-worker"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      ...
      command = [
        <<-EOT
          ...
          php artisan queue:work --queue=invoice,notification
        EOT
      ]
      secrets = [
        {
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"         = "/ecs/general-worker"
          "awslogs-region"        = var.aws_region
          "awslogs-stream-prefix" = "general-worker"
        }
      }
    }
  ])
...
resource "aws_ecs_capacity_provider" "general_worker_on_demand" {
  name = "${var.service_name}-${var.environment}-capacity-provider-general_worker-on-demand"

  auto_scaling_group_provider {
    auto_scaling_group_arn         = aws_autoscaling_group.general_worker_on_demand.arn
...
resource "aws_ecs_service" "general_worker" {
  name            = "${var.service_name}-${var.environment}-general-worker"
  cluster         = var.cluster_id
  task_definition = aws_ecs_task_definition.general_worker.arn
  desired_count   = var.general_worker_desired_count

  capacity_provider_strategy {
    capacity_provider = aws_ecs_capacity_provider.general_worker_on_demand.name
